...
export default {
    ...
    methods: {
        ...
        storeQuizInfo() {
            axios.post(`/api/quiz/info/store/typing-balloon?flag=tutorial`, this.typingBalloon)
                .then(() => {
            ...
            if (this.array[this.h].text.substring(0, 1) === char) {
                ...
                if (this.typingBalloon.score == (this.array.length + this.previousScore)) {
                    if (this.typingBalloon.score == 26) {
                        this.show = true
                        this.text = `Congratulations! You've got the perfect score. Your score is ${this.typingBalloon.score}.`
                        this.nextButtonText = 'Replay'
                        this.cancelButtonText = 'Exit'
                    }
                }

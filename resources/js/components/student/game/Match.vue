                    ...
                    <transition-group :name="'flip-list'">
                        <div :class="optionClasses.grid_class" class="d-flex flex-column align-items-center"
                            ...
                            <div :class="optionClasses.card_class" style="cursor: pointer"
                                ...
                                class="card card-body d-flex flex-row justify-content-center card-animation">
...
export default {
    ...
    computed: {
        ...
        optionClasses() {
            const gridClass = 'col-3';
            const containerClass = 'game-container-1';
            const cardClass = 'card-box-1';
            const imageClass = 'option-image-1';
            const rowClass = 'd-flex justify-content-center';
    ...
    methods: {
        ...
        async initQuizInfo() {
            ...
            const data = await axios.get(`/api/quiz-reports/get?gameId=${4}&flag=tutorial`)
            if (data.data) {
        ...
        storeQuizInfo() {
            axios.post(`/api/quiz/info/store/matching-game?flag=tutorial`, this.matchingGame)
                .then(() => {
                    this.getQuizInfo('tutorial')
                })
        },
        updateQuizInfo() {
            axios.post(`/api/quiz/info/update/${this.quizInfo.id}?gameId=4&flag=tutorial`, this.matchingGame)
        },
        nextLevel() {
        ...
        submit() {
            ...
            if (_.isEqual(JSON.stringify(matchA), JSON.stringify(matchB))) {
                ...
                if (this.matchingGame.score == 12) {
                    ...
                    swal.fire({
                        text: `Great. Try another combination.`,
                ...
                this.strikes.pop()
                this.strikes = [{ key: Math.floor(Math.random() * 100), icon: '🚫', guess: this.randomNumber }, ...this.strikes]
